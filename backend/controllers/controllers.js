...
export const uploadData = (req, res) => {
  uploadMultiple(req, res, async (err) => {
    ...
    const sql = `INSERT INTO images (four_image_array, nine_image_array, single_image, title, client_name, work, descp_one, project_title, descp_two, section_title, imp_features, section_title_two, imp_features_two, url, webUrl) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    ...
    db.query(
      sql,
      [JSON.stringify(fourImageArray), JSON.stringify(nineImageArray), singleFile, title, clientName, work, descpOne, projectTitle, descpTwo, sectionTitle, impFeatures, sectionTitleTwo, impFeaturesTwo, url, webUrl],
      (err, data) => {
        if (err) {
